...
def main_page():
    ...
    def register_user():
        ...
        def disab():
            ...
            emailid_entry.grid_forget()
            phno_entry.grid_forget()
            buutn.grid_forget()
        ...
        if re.fullmatch(r'[A-Za-z0-9@#$%^&+=]{8,}', password.get()):
            ...
            global bttnn
            disab()
            lbel = Label(screen1, text="Use Strong Password", font ="green", width='30')
            lbel.grid(row=1, column=1, columnspan=1)
            bttnn = Button(screen1, text="OK", width="15", command=reab)
            bttnn.grid(row=2, column=1, columnspan=1)
