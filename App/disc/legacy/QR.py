...
def CreateWidgets():
    ...
    screenx.entry = Entry(width=30, textvariable=qrInput)
    screenx.entry.grid(row=0, column=2, padx=5, pady=5, columnspan=2)
    label = Label(text="Enter Phno : ", bg="green")
    label.grid(row=1, column=1, padx=5, pady=5)
    screenx.entry = Entry(width=30, textvariable=qrphno)
    screenx.entry.grid(row=1, column=2, padx=5, pady=5, columnspan=2)
    label = Label(text="Enter Email : ", bg="green")
    label.grid(row=2, column=1, padx=5, pady=5)
    screenx.entry = Entry(width=30, textvariable=qrmail)
    screenx.entry.grid(row=2, column=2, padx=5, pady=5, columnspan=2)    
    label = Label(text="Event Name : ", bg="green")
    label.grid(row=3, column=1, padx=5, pady=5)    
    screenx.entry = Entry(width=30, textvariable=qrevent)
    screenx.entry.grid(row=3, column=2, padx=5, pady=5, columnspan=2)
    ...
    label = Label(text="QR Code : ", bg="green")
    label.grid(row=4, column=1, padx=5, pady=5)
    screenx.imageLabel = Label(screenx, background="green")
    screenx.imageLabel.grid(row=5, column=1, columnspan=3, padx=5, pady=5)
    image = Image.open("./resc/wait.jpg")
    image = image.resize((350, 350), Image.ANTIALIAS)
    image = ImageTk.PhotoImage(image)
    screenx.imageLabel.config(image=image)
    screenx.imageLabel.photo = image
...
screenx = tk.Tk()
screenx.title("QR Generator")        
#screenx.geometry("405x575")
screenx.geometry("600x600")
screenx.resizable(False, False)
screenx.config(background="green")
qrInput = StringVar()
qrphno = StringVar()
qrmail = StringVar()
workbook = Workbook()
